...
function try_sodium_manual($encrypted_full, $key) {
    ...
    // Try raw key
    try {
        // Key must be 32 bytes. If not, maybe md5?
        $k = $key;
    ...
    // Try raw key strict (if 32 bytes)
    try {
        if (strlen($key) === SODIUM_CRYPTO_SECRETBOX_KEYBYTES) {
            $pt = sodium_crypto_secretbox_open($msg, $nonce, $key);
            if ($pt !== false) return $pt;
        }
    } catch (\Exception $e) {}
...
function smart_decrypt($om, $value, $keys) {
    ...
    // Fallback: If default returned something non-numeric (e.g. string), return it
    if (isset($pt) && $pt) return $pt;
